...
仓库内置给 Streamlit 页面使用的后台工作流：

- `.github/workflows/web_quant_jobs.yml`：原漏斗与批量 AI 任务
- `.github/workflows/custom_trend25_jobs.yml`：`custom_trend25` 并行策略（独立队列）

- `Wyckoff Funnel` 页面：提交后台漏斗筛选任务（原链路）
- `Wyckoff Funnel` 页面（自定义并行策略 Tab）：提交 `custom_trend25` 独立任务
- `AI 分析` 页面：提交后台批量研报或输入预演任务
- 页面职责：触发工作流、轮询运行状态、读取 artifact 结果
- 后台职责：拉全量 OHLCV、跑筛选、调用模型、产出轻量 JSON 结果
...
| `GITHUB_ACTIONS_WORKFLOW_FILE` | 否 | 默认 `web_quant_jobs.yml`（原漏斗/批量 AI） |
| `GITHUB_ACTIONS_CUSTOM_TREND25_WORKFLOW_FILE` | 否 | 默认 `custom_trend25_jobs.yml`（自定义并行策略） |
| `GITHUB_ACTIONS_ALLOWED_USERS` | 否 | 逗号分隔白名单，支持 `user_id` 或 `email`；配置后仅白名单账号可触发后台任务 |
| `GITHUB_ACTIONS_ALLOWED_USER_IDS` | 否 | 兼容旧配置，语义同上（建议迁移到 `GITHUB_ACTIONS_ALLOWED_USERS`） |
